...
class Nosto_Seclib_File_ASN1
{
    ...
    */
    ...
    function _decodeOID($content)
    {
        static $eighty;
        if (!$eighty) {
            $eighty = new Nosto_Seclib_Math_BigInteger(80);
        }

        $oid = array();
        $pos = 0;
        $len = strlen($content);
        $n = new Nosto_Seclib_Math_BigInteger();
        while ($pos < $len) {
            $temp = ord($content[$pos++]);
            $n = $n->bitwise_leftShift(7);
            $n = $n->bitwise_or(new Nosto_Seclib_Math_BigInteger($temp & 0x7F));
            if (~$temp & 0x80) {
                $oid[] = $n;
                $n = new Nosto_Seclib_Math_BigInteger();
            }
    ...
    function _encodeOID($source)
    {
        ...
        if (!$mask) {
            $mask = new Nosto_Seclib_Math_BigInteger(0x7F);
            $zero = new Nosto_Seclib_Math_BigInteger();
            $forty = new Nosto_Seclib_Math_BigInteger(40);
        }
        ...
        $first = new Nosto_Seclib_Math_BigInteger($part1);
        $first = $first->multiply($forty);
        $first = $first->add(new Nosto_Seclib_Math_BigInteger($part2));
        ...
        foreach ($parts as $part) {
            if (!$part) {
                ...
                $temp = '';
                $part = new Nosto_Seclib_Math_BigInteger($part);
